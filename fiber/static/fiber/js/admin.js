...
(function($) { // start of jQuery noConflict mode
	...
	$(window).ready(function() {
		...
		var api_url_root =  $("body").data('api-url');
		Fiber.api_url_root = api_url_root?api_url_root:"/api/v2/";
		...
		Fiber.ImageSelectDialog = BaseFileSelectDialog.extend({
			...
			get_upload_path: function() {
				return Fiber.api_url_root + 'images/';
			},
		...
		Fiber.FileSelectDialog = BaseFileSelectDialog.extend({
			...
			get_upload_path: function() {
				return Fiber.api_url_root + 'files/';
			},
		...
		Fiber.PageSelectDialog = AdminRESTDialog.extend({
			...
			init_dialog_success: function(data) {
				...
				$.ajax({
					url: Fiber.api_url_root + 'pagetree/',
					type: 'GET',
		...
		var DroppableArea = Class.extend({
			...
			add_content_item: function(content_item_id) { // TODO: move to utils? (DRY)
				...
				$.ajax({
					url: Fiber.api_url_root + 'page_content_items/',
					type: 'POST',
					data: data,
			...
			move_content_item: function(fiber_item_data) {
				...
				Fiber.move_page_content_item(
					Fiber.api_url_root + 'page_content_items/' + fiber_item_data.page_content_item_id + '/move/',
		...
		var adminPage = {
			...
			init_page_tree: function() {
				...
				function handleMoveNode(event) {
					...
					$.ajax({
						url: Fiber.api_url_root + 'pages/' + info.moved_node.id + '/move_page/',
						type: 'POST',
						dataType: 'json',
								...
								buttons: {
									'Delete': {
										...
										click: function() {
											...
											$.ajax({
												url: Fiber.api_url_root + 'pages/' + node.id + '/',
												type: 'DELETE',
												data: {},
												success: function(data) {
							...
							buttons: {
								'Delete': {
									...
									click: function() {
										...
										$.ajax({
											url: Fiber.api_url_root + 'content_items/' + node.id + '/',
											type: 'DELETE',
											data: {},
											success: function(data) {
												reloadPage();
		...
		*/
		function reloadPage(params) {
			...
			if (!page_id) {
				...
				$.ajax({
					url: Fiber.api_url_root + 'pages/' + page_id + '/',
					type: 'GET',
					success: function(data) {
		...
		Fiber.FiberItem = Class.extend({
			...
			remove_from_page: function() {
				...
				$.ajax({
					url: Fiber.api_url_root + 'page_content_items/' + this.element_data.page_content_item_id + '/',
					type: 'DELETE',
					data: {},
					success: function(data) {
						reloadPage();
